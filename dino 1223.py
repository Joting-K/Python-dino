...
# 使用者介面 使用tk
nn = 1  #  遊玩順序
path = "C:/Users/okahe/Desktop/dinosaur-master"
                    ...
                    if over==False:
                        ...
                        if first_obs == True:
                            ...
                            if game_state == 'Watching':
                                score_obstacle_list.append(obstacle)
                            first_obs = False
                        else:
                            #计算障碍物间隔的时间
                            if addobstacleTimer>=1300:
                        ...
                        for i in range(len(score_obstacle_list)):
                            # 當聽覺關轉換到視覺關時: 在2秒內不會死
                            if game_state == "Watching":
                                if pygame.sprite.collide_rect(dinasaur, score_obstacle_list[i]):
                                    #if pygame.time.get_ticks() - watching_game_start_time >= 2000:
                                    print("you fail the game")
                                    over=True
                                    game_over()
                                    ...
                                    if (score_obstacle_list[i].rect.x + score_obstacle_list[i].rect.width) < dinasaur.rect.x:
                                        #加分
                                        #score += score_obstacle_list[i].getScore()
                                        score_obstacle_list[i].setPassState("Watching")
                        ...
                        # 處理視覺關加分  
                        for i in range(len(score_obstacle_list)):
                            if score_obstacle_list[i].getPassState()== "Watching" :
                                #加分
                                score += score_obstacle_list[i].getScore()
