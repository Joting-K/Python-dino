...
class GUI():
    def __init__(self):
        ...
        self.role_combobox = ttk.Combobox(self.gui, textvariable=tk.StringVar(), font=('微軟正黑體', 20, "bold"))
        ...
        self.type_combobox = ttk.Combobox(self.gui, textvariable=tk.StringVar(), font=('微軟正黑體', 20, "bold"))
    ...
    def do_ok(self):
        ...
        if self.type_combobox.get()=='視覺關':  # 視覺關
            ...
            class Obstacle:
                score=1 #分数
                pass_state = "" # 經過障礙物時，標註此障礙物是"Watching"or"Hearing"
                def setPassState(self, state): # 標註障礙物的"經過狀態下的關卡"的功能                    
                    if self.pass_state == "":
                ...
                '''
                #获取分数
                def minusgetScore(self):
                    self.score
                    tmp=self.score
                    if tmp==1:
                        self.minus_audio.play()
                    self.score=0
                    return tmp
                '''
            ...
            def Hear_game():    
                ...
                screen_h=pygame.display.Info().current_h
                # 載入聽覺關遊戲的圖片
                hear_img=pygame.image.load(os.path.join(mypath, "image\\hearing.png")).convert_alpha()
                # 繪製聽覺關遊戲的圖標在視窗中間
            ...
            def Watch_game():    
                ...
                screen_h = pygame.display.Info().current_h
                # 載入視覺關遊戲的圖片
                watch_img = pygame.image.load(os.path.join(mypath, "image\\watching.png")).convert_alpha()
                # 繪製視覺關遊戲的圖標在視窗中間
                ...
                while True:
                    ...
                    if over==False:
                            ...
                            if addobstacleTimer>=1300:
                                ...
                                if obstacle_crl > 50:
                                    ...
                                    if game_state == 'Hearing':
                                        hear_obstacle_list.append(obstacle)
                                        not_minus = False
                                    '''
                                #重置添加障碍物的时间
                                addobstacleTimer=0
                        ...
                        for i in range(len(watch_obstacle_list)):
                            ...
                            if game_state == "Hearing":
                                if (score_obstacle_list[i].rect.x + score_obstacle_list[i].rect.width) < dinasaur.rect.x:
                                    score_obstacle_list[i].setPassState("Hearing")
                        
                        '''
                        for i in range(len(hear_obstacle_list)):
                            if game_state == "Hearing":
                                print(not_minus)
                                if pygame.sprite.collide_rect(dinasaur, hear_obstacle_list[i]):
                                    not_minus = True
                                    print('not minus')
                                if not_minus == False:                                
                                    if (hear_obstacle_list[i].rect.x + hear_obstacle_list[i].rect.width) < dinasaur.rect.x:
                                        print('a')
                                        #if score > 0:
                                            #score -= hear_obstacle_list[i].minusgetScore() 
                                        hear_obstacle_list[i].setPassState("Hearing")
                        '''
                        ...
                        # 處理聽覺關扣分  
                        for i in range(len(hear_obstacle_list)):
                            if hear_obstacle_list[i].getPassState()== "Hearing" :
                                if score > 0:
                                    score -= hear_obstacle_list[i].minusgetScore()  
                        '''
